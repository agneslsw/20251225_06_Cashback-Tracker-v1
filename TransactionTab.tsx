            ...
            <tbody className="divide-y divide-[#e8e6e1]">
              {transactions.map(tx => (
                <tr key={tx.id} onClick={() => {
                  ...
                  <td className="px-4 py-4"><div className="text-xs font-normal truncate max-w-[100px]">{tx.description}</div><div className="flex gap-2 mt-1"><span className="text-[8px] text-[#b4a088] uppercase tracking-tighter">{cards.find(c => c.id === tx.cardId)?.name}</span><span className={`text-[8px] px-1.5 py-0.5 rounded-full border ${tx.status === 'Confirmed' ? 'text-green-600 bg-green-50' : tx.status === 'Pending' ? 'text-yellow-600 bg-yellow-50' : 'text-gray-500 bg-gray-50'}`}>{tx.status}</span></div></td>
              ...
              {transactions.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-4 py-10 text-center text-[#717171] italic text-xs">No records found.</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-[2px] z-[60] flex items-end sm:items-center justify-center p-4">
          <div className="bg-[#fcfbf7] w-full max-w-md flex flex-col h-[85vh] rounded-t-3xl border border-[#e8e6e1] animate-in slide-in-from-bottom">
            <div className="px-6 py-4 border-b border-[#e8e6e1] bg-white flex justify-between items-center"><h3 className="text-[11px] font-light uppercase tracking-wider">{editingId ? 'Edit Entry' : 'New Entry'}</h3><div className="flex gap-2"><label className="text-[8px] uppercase flex items-center gap-1">Redeem <input type="checkbox" checked={isRedemption} onChange={e => (setIsRedemption(e.target.checked), e.target.checked && setIsCashbackIn(false))} /></label><label className="text-[8px] uppercase flex items-center gap-1">Credit <input type="checkbox" checked={isCashbackIn} onChange={e => (setIsCashbackIn(e.target.checked), e.target.checked && setIsRedemption(false))} /></label></div></div>
            <div className="flex-1 overflow-y-auto p-6 space-y-4 no-scrollbar">
              <div><label className="block text-[9px] uppercase tracking-widest text-[#717171] mb-1">Date</label><input type="date" value={date} onChange={e => setDate(e.target.value)} className="w-full bg-white border border-[#e8e6e1] rounded-lg px-2 py-1 text-[11px] outline-none" /></div>
              <div><label className="block text-[9px] uppercase tracking-widest text-[#717171] mb-1">Description</label><input value={desc} onChange={e => setDesc(e.target.value)} className="w-full bg-white border border-[#e8e6e1] rounded-lg px-2 py-1 text-[11px] outline-none" /></div>
              <div><label className="block text-[9px] uppercase tracking-widest text-[#717171] mb-1">Credit Card</label><select value={cardId} onChange={e => (setCardId(e.target.value), !isRedemption && !isCashbackIn && setRebateOverride(calculateSuggestedRebate(e.target.value, selectedMerchants).toString()))} className="w-full bg-white border border-[#e8e6e1] rounded-lg px-2 py-1 text-[11px] outline-none"><option value="">Select Card</option>{cards.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
              <div><label className="block text-[9px] uppercase tracking-widest text-[#717171] mb-1">Amount</label><input type="number" step="0.01" value={amount} onChange={e => setAmount(e.target.value)} className="w-full bg-white border border-[#e8e6e1] rounded-lg px-2 py-1 text-[11px] outline-none" /></div>
              <div>
                <label className="block text-[9px] uppercase tracking-widest text-[#717171] mb-1">Status</label>
                <select value={status} onChange={e => setStatus(e.target.value as Status)} className="w-full bg-white border border-[#e8e6e1] rounded-lg px-2 py-1 text-[11px] outline-none">
              ...
              {!isRedemption && !isCashbackIn && (
                <>
                  <div><label className="block text-[9px] uppercase tracking-widest text-[#717171] mb-1">Merchant Types</label><div className="flex flex-wrap gap-1">{merchantTypes.map(m => <button key={m} onClick={() => { const next = selectedMerchants.includes(m) ? selectedMerchants.filter(i => i !== m) : [...selectedMerchants, m]; setSelectedMerchants(next); setRebateOverride(calculateSuggestedRebate(cardId, next).toString()); }} className={`px-2 py-0.5 rounded-full text-[9px] border transition-all ${selectedMerchants.includes(m) ? 'bg-[#b4a088] text-white border-[#b4a088]' : 'bg-white text-[#717171] border-[#e8e6e1]'}`}>{m}</button>)}</div></div>
                  <div><label className="block text-[9px] uppercase tracking-widest text-[#717171] mb-1">Rebate (%)</label><input type="number" step="0.01" value={rebateOverride} onChange={e => setRebateOverride(e.target.value)} className="w-full bg-white border border-[#e8e6e1] rounded-lg px-2 py-1 text-[11px] outline-none" /></div>
                </>
              )}
            </div>
            <div className="p-4 border-t bg-white flex gap-3"><button onClick={() => setShowModal(false)} className="flex-1 px-4 py-2 text-xs border rounded-xl text-[#717171] border-[#e8e6e1]">Cancel</button><button onClick={handleSave} className="flex-[2] px-4 py-2 text-xs bg-[#b4a088] text-white rounded-xl active:opacity-80">Save Entry</button></div>
          </div>
        </div>
      )}
    </div>
